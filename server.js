...
/* As três camadas na porta: classifica a requisição por origem, responde
   preflight e RECUSA escrita vinda de outro site. Ele NÃO é login — o cockpit
   não tem mais login, e mesmo quando tinha, o portão de página e este bloco
   respondiam a perguntas diferentes. Ele fica porque é o que protege
   `/api/claude/run/:id/approve`, que escreve em fluxo de produção.
   `guarda.lerPareamento(__dirname)` lê `.agente/pareamento.json` com try/catch
   e devolve `null` quando o arquivo não existe, então o bloco continua
   funcionando sem a cerimônia que escrevia esse arquivo (`pareamento.js`, que
   saiu junto com a camada de login). Sem pareamento, só cliente local entra. */
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    /* NÃO HÁ PORTÃO DE LOGIN AQUI, e a ausência é a decisão. O cockpit deixou de
     * ser produto: as quatro abas são de uma pessoa, nesta máquina. O que
     * protege o painel é o `127.0.0.1` do `listen` lá embaixo mais o bloco do
     * guarda acima, que é o que ele sempre foi — a página nunca foi o cadeado,
     * e agora não existe nem a página de conta para onde redirecionar. */
    ...
    /* SÃO QUATRO PORTAS, E SÓ QUATRO. Aqui moravam `/conta`, `/entrar` e
     * `/integracoes`, mais as rotas que as alimentavam: `/api/perfil/*` (login,
     * equipe, convite, organização), `/api/cofre` (a chave do n8n gravada pela
    ...
    if (p.startsWith("/api/upgrade/arquivo/")) {
      ...
      /* O TIPO DO ANEXO NAO PODE SER O TIPO QUE ELE PEDE. `anexos.TIPOS` aceita
         ...
         com `guarda.classificar` dizendo `propria`, que e a classe que pode
         escrever. Dali o script chama `POST /api/claude/run/:id/approve` (escreve
         em fluxo de producao) e `.../retry` (mensagem real para um lead, sem
         desfazer). O `POST /api/cofre` citado aqui antes era um terceiro alvo e
         saiu com a camada de login: hoje a chave do n8n so vem do `.env`, e
         nao ha cookie de sessao para o documento carregar — o que NAO diminui
         nada, porque as duas rotas que sobraram sao as irreversiveis. O vetor e
         a rotina que o
...
server.listen(PORT, "127.0.0.1", () => {
  ...
  /* O boot não fala mais do estado do login, e não fala porque não há login: as
     três frases que ficavam aqui existiam para que "o painel me redirecionou
     para a tela de conta" fosse reconhecível. Sem portão, uma linha sobre ele
     seria pior que o silêncio — mandaria procurar uma tela que não existe. */
